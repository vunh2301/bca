...
class Menu extends \Phalcon\Mvc\Model
{
	...
	public function showTree($typeId)
	{
		...
		foreach($tree as $node){
			...
			$result .= '<li class="dd-item" data-id="'.$node_id.'">';
			$result .= '<div class="dd-handle">'.$node_name.'<span class="glyphicon glyphicon-trash pull-right" aria-hidden="true" style="cursor:pointer"></span></div>';
			++$counter;
		}
		$result .= str_repeat('</li></ol>',$node_depth).'</li>';
		$result='<div class="dd" data-id="1">'.$result."</div>";
		return $result;
